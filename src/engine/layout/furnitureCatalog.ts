...
function placeholderSprite(w: number, h: number, color: string): SpriteData {
  ...
  for (let r = 0; r < h * 16; r++) {
    ...
    for (let c = 0; c < w * 16; c++) {
      const isBorder = r === 0 || r === h * 16 - 1 || c === 0 || c === w * 16 - 1
      row.push(isBorder ? '#333333' : color)
    }
    sprite.push(row)
  }
  return sprite
...
export type FurnitureCategory = 'desks' | 'chairs' | 'storage' | 'decor' | 'electronics' | 'wall' | 'misc' | 'dating'
...
export const FURNITURE_CATALOG: CatalogEntryWithCategory[] = [
  ...
  { type: FurnitureType.FLOWER_POT,   label: 'Flower Pot',   footprintW: 1, footprintH: 1, sprite: placeholderSprite(1, 1, '#FF69B4'), isDesk: false, category: 'decor' },
  { type: FurnitureType.PHOTO_FRAME,  label: 'Photo Frame',  footprintW: 1, footprintH: 1, sprite: placeholderSprite(1, 1, '#FFD700'), isDesk: false, category: 'wall', canPlaceOnWalls: true },
  { type: FurnitureType.STAGE_LIGHT,  label: 'Stage Light',  footprintW: 1, footprintH: 1, sprite: placeholderSprite(1, 1, '#FFA500'), isDesk: false, category: 'decor' },
